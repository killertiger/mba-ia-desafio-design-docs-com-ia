...
A motivação para adotar autenticação em nível de payload — e não apenas TLS — surge de experiência operacional concreta: há precedente de cliente que vazou uma secret de integração em logs de aplicação, o que expõe a necessidade de isolar o raio de blast por endpoint. O ponto central é que a plataforma passa a expor eventos com dados de pedidos para um endpoint fora da sua infraestrutura, e o cliente precisa conseguir validar que a requisição veio realmente da plataforma.
...
- O mecanismo deve ser compatível com bibliotecas disponíveis em qualquer linguagem de programação, pois qualquer cliente sério já dispõe de biblioteca para HMAC-SHA256.
- Todas as URLs de destino devem usar TLS obrigatoriamente — URLs `http://` devem ser rejeitadas em validação de schema.
...
Escolhida a **Opção A — HMAC-SHA256 com secret única por endpoint e grace period de 24 horas**: assinatura HMAC-SHA256 sobre o corpo do request, secret por endpoint e suporte a rotação com grace period de 24 horas.
...
Cada request de webhook inclui os headers: `X-Event-Id` (UUID do evento para idempotência), `X-Signature` (HMAC-SHA256 sobre o corpo), `X-Timestamp` (timestamp de envio para detecção opcional de replay attack), `X-Webhook-Id` (UUID do endpoint, para clientes com múltiplos endpoints cadastrados), e `Content-Type: application/json`.

A secret é gerada pela plataforma e devolvida ao cliente no momento da criação do webhook.
...
**Prós:**
- Vazamento de uma secret isola o impacto a um único endpoint, cenário confirmado por caso real na base de clientes.
...
**Contras:**
- Vazamento de uma única secret compromete todos os clientes e endpoints simultaneamente.
- O caso real documentado de secret vazada em log de aplicação causaria dano total neste modelo.
...
A adoção desta arquitetura de segurança cria um gate de processo explícito: há 2 dias úteis reservados para revisão do código de segurança antes do deploy, com foco específico na lógica HMAC e na geração de secrets. Este requisito de revisão deve ser registrado no processo de desenvolvimento da feature — não é opcional sob pressão de prazo.
...
[PRECISA DE INFORMAÇÃO: Qual é a política de comprimento mínimo e fonte de entropia para as secrets geradas automaticamente? Está definido que a secret é gerada pela plataforma, mas não há especificação de requisitos de entropia, e não há precedente no código atual — o projeto usa bcrypt para senhas e valida `JWT_SECRET` com no mínimo 16 caracteres (`src/config/env.ts:8`), sem geração aleatória de segredos.]

[PRECISA DE INFORMAÇÃO: A secret é recuperável via GET após a criação, ou é entregue apenas uma vez? Está definido que ela é devolvida na criação, mas não a política de exposição posterior. A prática recomendada é entrega única — recuperação exigiria nova rotação.]

[PRECISA DE INFORMAÇÃO: Como o sistema deve se comportar quando um cliente ainda usa a secret anterior após o grace period de 24 horas expirar? O erro deve ser explícito (HTTP 401 com código `WEBHOOK_SECRET_EXPIRED`) ou silencioso? Este comportamento de borda não está definido.]

[PRECISA DE INFORMAÇÃO: A proteção contra replay attack via `X-Timestamp` ficará para implementação futura obrigatória ou é opcional para o cliente? O header está definido, mas não está definido se a plataforma fará validação da janela temporal ou apenas documentará a semântica para os clientes.]
